...
LciaMethod = namedtuple(
    "LciaMethod",
    (
        "IMPACT_CATEGORY_UUID",
        "FILENAME",
        "IMPACT_METHOD",
        "IMPACT_CATEGORY",
        "REFERENCE_UNIT",
...
def generate_impacts_list():
    ...
    for filename in methods:
        ...
        try:
            entries.append(
                LciaMethod(
                    firstline["IMPACT_CATEGORY_UUID"],
                    filename,
                    firstline["IMPACT_METHOD"],
                    firstline["IMPACT_CATEGORY"],
                    firstline["REFERENCE_UNIT"],
                )
            )
    ...
    with open(os.path.join(REF_DIR, "impact_categories.csv"), "w") as fp:
        wr = csv.DictWriter(
            fp, fieldnames=LciaMethod._fields, quoting=csv.QUOTE_ALL, delimiter=";"
        )  # _fields is not really protected
